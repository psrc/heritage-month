...
```{r}
library(stringr)
Asian_df_max_tract<-Asian_df_tract%>% select('GEOID', 'label', 'estimate', 'moe')%>% filter(label!='Estimate!!Total:') %>% group_by(GEOID)%>%top_n(1, estimate)%>%mutate(label_short=str_replace_all(label, 'Estimate!!Total:!!', replacement=''))%>%
             mutate(label_short_na=if_else(estimate>300, "not enough data", label_short ))%>%
  mutate(top_group= as.factor(label_short_na))%>%mutate('GEOID'=as.character(GEOID))%>%select(GEOID, top_group)
```
...
create_tract_map_categorical <- function(tbl, tract.lyr,
  ...
  pal <- leaflet::colorFactor(palette="Spectral", domain = c.layer$top_group)


  labels <- paste0("Census Tract: ", c.layer$geoid, '<p></p>',
                   'Top Asian Alone Group: ', c.layer$top_group %>% lapply(htmltools::HTML))
