...
Hispanic_df <- format_estimates(get_acs_recs(geography = 'county',
             ...
             years=c(2020),
             acs.type = 'acs5')%>%filter(name=='Region'), type='share')%>%mutate(Ethnicity='All Hispanic/Latinx')


black_df <- format_estimates(get_acs_recs(geography = 'county',
             table.names = c('B08105B'),
             years=c(2020),
             acs.type = 'acs5')%>%filter(name=='Region'), type='share')%>%mutate(Ethnicity='Black or African American, includes Hispanic/Latinx')



Asian_df <- format_estimates(get_acs_recs(geography = 'county',
             table.names = c('B08105D'),
             years=c(2020),
             acs.type = 'acs5')%>%filter(name=='Region'), type='share')%>%mutate(Ethnicity='Asian, includes Hispanic/Latinx')


white_Hispanic_df<-rbind(Hispanic_df, white_df, black_df, Asian_df)%>%filter(label!='Estimate!!Total:' & label!='Estimate!!Total:!!Female:' & label!= 'Estimate!!Total:!!Male:')%>%filter(label!='Estimate!!Total:!!Car, truck, or van - drove alone')%>%mutate(label=str_remove(label, 'Estimate!!Total:!!'))%>%mutate(label=str_wrap(label, width=20))%>%mutate(Ethnicity=str_wrap(Ethnicity, width=25))




p<-create_column_chart(t=white_Hispanic_df, w.x='label', w.y='Region_share', f='Ethnicity', w.moe='Region_share_moe', est.type='percent',  w.color = 'psrc_dark')+theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+theme(legend.text = element_text(size=14,color="#4C4C4C"))+
theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+theme(axis.text.y = element_text(size=14,color="#4C4C4C"))
ggsave('C:/GitHub/heritage-month/hispanic_heritage2/means_transport.jpeg', p, device='jpeg', width=15, height=10, dpi=72)
