...
write.csv(white_Hispanic_df,'C:/GitHub/heritage-month/hispanic_heritage2/hhsize.csv')


```

```{r}
race_rent_obj<-get_psrc_pums(span=5, dyear=2020, level="h", vars=c("HRACE","GRPIP"))%>% mutate(counts_cut_number   = cut_interval(GRPIP, n = 4))


race_rent<-psrc_pums_mean(race_rent_obj,stat_var="GRPIP", group_var = "HRACE")%>%filter(HRACE=='Hispanic or Latino' | HRACE=='White alone')

race_rent_count<-psrc_pums_count(race_rent_obj, group_var=c('HRACE', 'counts_cut_number'))%>%drop_na(counts_cut_number)%>%filter(counts_cut_number!='Total')


race_rent_count$counts_cut_number_f=factor(race_rent_count$counts_cut_number, levels=c('[1,26]', '(26,51]', '(51,76]','(76,101]'))
...
p<-create_column_chart(t=race_rent_count, w.x='counts_cut_number_f', w.y= 'count', f='HRACE', w.moe='count_moe', est.type='number', w.color = 'psrc_dark' )
...
write.csv(white_Hispanic_df,'C:/GitHub/heritage-month/hispanic_heritage2/housecost.csv')
