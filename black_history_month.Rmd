...
nativity_df_Black_formatted<-nativity_df_Black %>% select(name, estimate, moe) 
...
women_birth_df_white<-get_acs_recs(geography = 'county',
             table.names = c('B13002H'),
             years=c(2019),
             acs.type = 'acs1') 


women_birth_df_black_white<- merge(women_birth_df_Black, women_birth_df_white, by=c('label','name')) %>%filter(label=='Estimate!!Total:' |label =='Estimate!!Total:!!Women who had a birth in the past 12 months:')%>% filter(name=='Region')

write.table(women_birth_df_black_white,"clipboard", sep='\t', row.names=FALSE )
#7% of Black women, 5% of White Women
```
